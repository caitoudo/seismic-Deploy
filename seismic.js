...
console.log(banner);
console.log(centerText("===- Seismic自动交互 -==="));
console.log(centerText("⌞👤 修改:crypto苦行僧⌝ \n"));
console.log(centerText("⌞ https://x.com/caitoudu⌝ \n"));
...
function loadConfigFromEnv() {
  ...
  if (!process.env.PRIVATE_KEYS) {
    throw new Error("请在.env文件中配置PRIVATE_KEYS");
  }
  
  const privateKeys = process.env.PRIVATE_KEYS.split(',');
  const proxyStrings = process.env.PROXIES ? process.env.PROXIES.split('|') : [];
  ...
  privateKeys.forEach((privateKey, index) => {
    try {
      ...
      const proxyString = proxyStrings[index] || '';
      ...
      console.log(`${colors.green}✅ 已加载账号 ${index + 1}: ${wallet.address}${colors.reset}`);
      if (proxyConfig) {
      ...
      console.error(`${colors.red}❌ 加载账号 ${index + 1} 时出错: ${error.message}${colors.reset}`);
    }
  });
...
async function main() {
  try {
    displaySection("加载配置");
    const accounts = loadConfigFromEnv();

    if (accounts.length === 0) {
      console.error(`${colors.red}❌ 没有加载任何账号！${colors.reset}`);
      console.log(`请在项目根目录创建.env文件，并添加以下配置：`);
      console.log(`PRIVATE_KEYS=私钥1,私钥2,私钥3`);
      console.log(`PROXIES=代理1|代理2|代理3 (可选)`);
      console.log(`代理格式: ip:port 或 ip:port:username:password`);
      rl.close();
      return;
    }

    console.log(`\n${colors.cyan}📝 已加载 ${colors.yellow}${accounts.length}${colors.cyan} 个账号${colors.reset}`);
    ...
    try {
      for (const account of accounts) {
        ...
        const { contractAddress, abi } = await deployTokenContract(account.wallet.privateKey, account.provider);
